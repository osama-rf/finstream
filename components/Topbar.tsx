  ...
  return (
    ...
    >
      <div className="flex h-16 items-center gap-4 px-4 sm:px-6">
        {/* Logo */}
        <Link href="/control-center" className="flex shrink-0 items-center">
          <Image src="/logo.png" alt="ركائز" width={34} height={40} priority className="sidebar-logo h-9 w-auto object-contain" />
        </Link>
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                href={item.path}
                className={`flex items-center gap-2 whitespace-nowrap rounded-[10px] px-3.5 py-2 text-sm font-medium font-arabic transition-all ${
                  isActive
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute end-0 top-full z-50 mt-2 min-w-56 rounded-xl border border-[var(--border)] bg-[var(--popover)] py-1 shadow-[var(--shadow-soft)]">
                  ...
                  <Link href="/settings" onClick={() => setMenuOpen(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-[var(--muted)]">
              ...
              return (
                <li key={item.path}>
                  <Link
                    ...
                    className={`flex items-center gap-3 rounded-[10px] px-3.5 py-2.5 text-sm font-medium font-arabic transition-all ${
                      isActive
                        ? "bg-[var(--primary)] text-[var(--primary-foreground)] shadow-[var(--shadow-soft)]"
                        : "text-[var(--muted-foreground)] hover:bg-[var(--accent)] hover:text-[var(--primary)]"
                    }`}
