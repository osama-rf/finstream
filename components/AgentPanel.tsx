...
const ATTENTION = {
  ar: {
    ...
    low:    { label: "وضع مستقر",     color: "var(--primary)",     bg: "color-mix(in srgb, var(--primary) 10%, transparent)" },
  },
  en: {
    ...
    low:    { label: "Stable",          color: "var(--primary)",     bg: "color-mix(in srgb, var(--primary) 10%, transparent)" },
  },
};
...
function ActionItem({ item, icon: Icon, color, routes }: {
  ...
  const inner = (
    <div className={`group flex items-center gap-3 rounded-[10px] border border-[var(--border)] bg-[var(--card)] px-4 py-3 transition-all ${href ? "cursor-pointer hover:border-[var(--primary)] hover:bg-[var(--surface)]" : ""}`}>
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            {report.actions_taken.map((action, i) => (
              <div key={i} className="flex items-start gap-3 rounded-[10px] border border-[var(--border)] bg-[color:color-mix(in_srgb,var(--primary)_6%,var(--card))] px-4 py-3">
                <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5 text-[var(--primary)]" />
      ...
      {report.next_steps.length > 0 && (
        <div className="space-y-1.5">
          ...
          <div className="space-y-2.5 px-1">
            {report.next_steps.map((s, i) => <ActionItem key={i} item={s} icon={ArrowLeft} color="var(--primary)" routes={routes} />)}
          </div>
        </div>
      )}
    </div>
      ...
      <DialogContent className="flex flex-col gap-0 p-0 max-h-[95dvh] w-[95vw] rounded-[16px] sm:max-w-2xl sm:max-h-[80vh]" dir={dir}>
        <DialogHeader className="shrink-0 border-b border-[var(--border)] px-5 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] bg-[color:color-mix(in_srgb,var(--primary)_10%,transparent)]">
        ...
        <div className="shrink-0 flex items-end gap-2 border-t border-[var(--border)] px-4 py-3">
          <textarea
            ...
            rows={1}
            disabled={isLoading}
            className="flex-1 resize-none rounded-[10px] border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:border-[var(--primary)] focus:outline-none disabled:opacity-50"
            style={{ maxHeight: "100px" }}
          />
          <Button size="sm" onClick={() => void sendMessage(input)} disabled={isLoading || !input.trim()}
            className="h-9 w-9 shrink-0 rounded-[10px] p-0 bg-[var(--primary)] text-[var(--primary-foreground)] hover:bg-[var(--primary-hover)]">
    ...
    <>
      <div className="rounded-[16px] border border-[var(--border)] bg-[var(--card)] overflow-hidden" dir={dir}>
        <div className="flex flex-wrap items-start gap-3 border-b border-[var(--border)] px-4 py-4 sm:px-5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[color:color-mix(in_srgb,var(--primary)_10%,transparent)]">
            <Bot className="h-4 w-4 text-[var(--primary)]" />
          </div>
          <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={() => void loadReport(true)} disabled={loading}
              className="flex items-center gap-1 rounded-[8px] border border-[var(--border)] px-2.5 py-1.5 text-xs text-[var(--muted-foreground)] transition-colors hover:border-[var(--primary)] hover:text-[var(--primary)] disabled:opacity-40 sm:gap-1.5 sm:px-3 sm:text-sm">
              <RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} />
              <span className="hidden sm:inline">{ui.refresh}</span>
            </button>
            <button onClick={() => openChat()}
              className="flex items-center gap-1 rounded-[8px] border border-[var(--border)] bg-[var(--surface)] px-2.5 py-1.5 text-xs text-[var(--foreground)] transition-colors hover:border-[var(--primary)] hover:text-[var(--primary)] sm:gap-1.5 sm:px-3 sm:text-sm">
          ...
          {loading ? (
            ...
            <div className="flex items-center gap-2.5 rounded-[10px] border border-[var(--border)] bg-[var(--surface)] px-4 py-3">
              ...
              <p className="text-sm text-[var(--muted-foreground)]">
                {ui.error}{" "}
                <button onClick={() => void loadReport()} className="text-[var(--primary)] hover:underline">{ui.retry}</button>
              </p>
            </div>
          ) : report ? (
            ...
            {roleActions.map((action: any) => {
              ...
              return (
                <button key={action.label} onClick={() => openChat(action.prompt)}
                  className="flex items-center gap-1.5 rounded-[10px] border border-[var(--border)] bg-[var(--surface)] px-2.5 py-1.5 text-xs text-[var(--foreground)] transition-all hover:border-[var(--primary)] hover:text-[var(--primary)] sm:px-3.5 sm:py-2 sm:text-sm">
